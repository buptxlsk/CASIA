# -*- coding: utf-8 -*-
"""
Created on Mon Dec  9 21:53:47 2024
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.set_origin_button = QPushButton("将目前位置存为物理原点", self.central_widget)
        self.set_origin_button.clicked.connect(self.set_physical_origin)
        ...
        '''
        self.clear_lines_button = QPushButton("清除线条", self.central_widget)
        self.clear_lines_button.clicked.connect(self.clear_lines)
        self.layout.addWidget(self.clear_lines_button,1,6)
        '''
        '''
        # 添加旋转按钮控件
        self.rotate_x_dial = QDial(self.central_widget)
        self.rotate_x_dial.setWrapping(True)
        self.rotate_x_dial.setRange(0, 360)
        self.rotate_x_dial.setValue(0)
        self.rotate_x_dial.setNotchesVisible(True)
        self.rotate_x_dial.valueChanged.connect(self.rotate_x)
        
        self.rotate_y_dial = QDial(self.central_widget)
        self.rotate_y_dial.setWrapping(True)
        self.rotate_y_dial.setRange(0, 360)
        self.rotate_y_dial.setValue(0)
        self.rotate_y_dial.setNotchesVisible(True)
        self.rotate_y_dial.valueChanged.connect(self.rotate_y)
        
        self.rotate_z_dial = QDial(self.central_widget)
        self.rotate_z_dial.setWrapping(True)
        self.rotate_z_dial.setRange(0, 360)
        self.rotate_z_dial.setValue(0)
        self.rotate_z_dial.setNotchesVisible(True)
        self.rotate_z_dial.valueChanged.connect(self.rotate_z)
        
        self.layout.addWidget(self.rotate_x_dial, 4, 1, 2, 1)
        self.layout.addWidget(self.rotate_y_dial, 1, 4, 2, 1)
        self.layout.addWidget(self.rotate_z_dial, 1, 1, 2, 1)'''
    ...
    def init_render_windows(self):
        ...
        self.render_window_interactor_axial2 = self.render_window_axial.GetInteractor()
        ...
        '''
        self.key_render_window_3d.self.vtk_
        self.key_renderer_3d = vtk.vtkRenderer()
        self.key_render_window_3d.'''
        ...
        #self.render_window_interactor_axial.AddObserver("LeftButtonPressEvent", self.

        '''
        def on_mouse_fwd(self, obj, event):
            print("WTF")
            
        def on_mouse_bwd(self, obj, event):
            print("FU")'''
    ...
    def create_menu(self):
        ...
        '''
        disable_mark_action = operation_menu.addAction("Disable Mark Point")
        disable_mark_action.triggered.connect(self.disable_marking)'''


        #mark_current_action = operation_menu.addAction("Mark Current Position")
        #mark_action.triggered.connectc(self.mark_current_point)
    ...
    def mark_point(self):
        if self.marking:
            ...
            physical_pos = self.update_physical_position_label_map(world_pos[0], world_pos[1], world_pos[2])
    ...
    def enable_erasing(self):
        ...
        '''
        self.set_erase_cursor(self.render_window_interactor_axial) #将光标设为圆形
        self.set_erase_cursor(self.render_window_interactor_coronal)
        self.set_erase_cursor(self.render_window_interactor_sagittal)'''
            ...
            if renderer is not None:
                ...
                if picked_actors:
                    for picked_actor in picked_actors:
                        if isinstance(picked_actor, vtk.vtkActor) and picked_actor.GetProperty().GetColor() == (1, 0, 0):
                            # 查找并删除表格中对应的点
                            pos = picked_actor.GetCenter()
                            self.marked_points = [mp for mp in self.marked_points if mp[1] != pos]
    ...
    def set_erase_cursor(self):
        ...
        if self.erasing:
            ...
            zoom_factor = camera.GetParallelScale()
            diameter = self.erase_radius * 2 / zoom_factor'''
        diameter = self.erase_radius
    ...
    def clear_erase_cursor(self):
        QApplication.restoreOverrideCursor()
        #self.render_window_interactor_axial.restoreOverrideCursor()
        #self.render_window_interactor_coronal.restoreOverrideCursor()
        #self.render_window_interactor_sagittal.restoreOverrideCursor()
    ...
    def set_SR(self): # 将目前位置保存为SR
        ...
        # physical_pos = self.update_physical_position_label(current_pos[0], current_pos[1], current_pos[2])
        physical_pos = self.update_physical_position_label_plus(current_pos[0], current_pos[1], current_pos[2])
        self.SR = ("SR", current_pos, current_angle, physical_pos)
        self.key_points.append(self.SR)
    ...
    def set_AODA(self): # 将目前位置保存为AODA
        ...
        # physical_pos = self.update_physical_position_label(current_pos[0], current_pos[1], current_pos[2])
        physical_pos = self.update_physical_position_label_plus(current_pos[0], current_pos[1], current_pos[2])
        self.AODA = ("AODA", current_pos, current_angle, physical_pos)
        self.key_points.append(self.AODA)
    ...
    def set_ANS(self): # 将目前位置保存为ANS
        ...
        # physical_pos = self.update_physical_position_label(current_pos[0], current_pos[1], current_pos[2])
        physical_pos = self.update_physical_position_label_plus(current_pos[0], current_pos[1], current_pos[2])
        self.ANS = ("ANS", current_pos, current_angle, physical_pos)
        self.key_points.append(self.ANS)
    ...
    def set_HtR(self): # 将目前位置保存为HtR
        ...
        # physical_pos = self.update_physical_position_label(current_pos[0], current_pos[1], current_pos[2])
        physical_pos = self.update_physical_position_label_plus(current_pos[0], current_pos[1], current_pos[2])
        self.HtR = ("HtR", current_pos, current_angle, physical_pos)
        self.key_points.append(self.HtR)
    ...
    def set_HtL(self): # 将目前位置保存为HtL
        ...
        # physical_pos = self.update_physical_position_label(current_pos[0], current_pos[1], current_pos[2])
        physical_pos = self.update_physical_position_label_plus(current_pos[0], current_pos[1], current_pos[2])
        self.HtL = ("HtL", current_pos, current_angle, physical_pos)
        self.key_points.append(self.HtL)
    ...
    def show_marked_points(self):
        ...
        '''
        marker_button = QPushButton("显示选择的点", dialog)
        marker_button.clicked.connect(lambda: self.select_two_points(table))'''
    ...
    def select_two_points(self, table):
        ...
        # 在这里调用你的函数，并传递选中的点数据
        #self.process_selected_points(point1, point2)
    ...
    def select_three_points(self, table):
        ...
        #def measure_mark_point_distance(self,point1,point2,drawing):
    ...
    def export_to_excel(self):
        ...
        if file_path:
            ...
            print(data[-1])
            df = pd.DataFrame(data, columns=["Name", "X", "Y", "Z", "Angle X", "Angle Y", "Angle Z", "Physical X","Physical Y","Physical Z"])
    ...
    def show_minimenu(self, obj, event):
        ...
        if not interactor.GetControlKey():  # 检查是否按下了 Ctrl 键，如按下就不显示小菜单
            ...
            mark_action.triggered.connect(self.enable_marking)
            '''
            disable_mark_action = context_menu.addAction("Disable Mark Point")
            disable_mark_action.triggered.connect(self.disable_marking)'''

            #mark_current_action = context_menu.addAction("Mark Current Position")
            #mark_action.triggered.connectc(self.mark_current_point)
    ...
    def show_slice_position_in_3d(self):
        ...
        pos = self.rotate_coordinate(x,y,z,
                                     ...
                                     self.rotate_z_input.value(), True)
        x = pos[0]
        y = pos[1]
        z = pos[2]
    ...
    def save_vtk_render_window(self, vtk_widget, file_path):
        ...
        #window_to_image_filter.SetInputBufferTypeToRGBA()  # Ensure alpha channel is captured
    ...
    def open_files(self):
        ...
        if file_dialog.exec():
            ...
            if selected_files:
                vtk_image = self.load_dicom_files(selected_files)
                if vtk_image:
    ...
    def load_dicom_files(self, filenames):
        ...
        self.image_orientation_patient = dicom_data.ImageOrientationPatient
        self.image_position_patient = dicom_data.ImagePositionPatient
        #self.patient_orientation = dicom_data.PatientOrientation
        self.slice_thickness = dicom_data.SliceThickness
        self.pixel_spacing = dicom_data.PixelSpacing

        #print(self.patient_orientation)

        #print(dicom_data.SliceThickness)
        #print(dicom_data.PixelSpacing)
        del(reader)
        del(dicom_io)
        del(dicom_data)
        return vtk_image
    ...
    def itk_to_vtk_image(self, itk_image):
        ...
        # 交换 X 和 Y 轴
       # itk_array = np.swapaxes(itk_array, 1, 2)
        ...
        '''
        data_array_x = vtk_data_array[1]
        data_array_y = vtk_data_array[0]
        vtk_data_array[0] = data_array_x
        vtk_data_array[1] = data_array_y
        '''
    ...
    def visualize_vtk_image(self, vtk_image):
        ...
        if not self.first_open:
            ...
            self.angles.clear()
            self.markers.clear() # 储存现在坐标在三视图上的红点actor
            self.lines.clear() # 储存线条actor
        ...
        '''
        self.rotate_x_quick(90)
        self.rotate_y_quick(180)'''

        # 3D 渲染部分

        # 创建变换对象，用于反转 X 轴
        transform = vtk.vtkTransform()
        transform.Scale(-1, 1, 1)  # 反转 X 轴

        # 应用变换到 vtk_image
        '''
        transform_filter = vtk.vtkTransformFilter()
        transform_filter.SetTransform(transform)
        transform_filter.SetInputData(self.flipped_image)
        transform_filter.Update()
        
        transformed_vtk_image = transform_filter.GetOutput()'''
       ...
       # self.renderer_3d.AddActor(axes)
        ...
        #transformed_vtk_image = self.flip_vtk_image(self.flipped_image,0)

        volume_mapper = vtk.vtkGPUVolumeRayCastMapper()
        #volume_mapper.SetInputData(transformed_vtk_image)
        ...
        #       color_function.AddRGBPoint(64, 1.0, 0.5, 0.3)
        #       color_function.AddRGBPoint(128, 1.0, 0.5, 0.3)
        #       color_function.AddRGBPoint(192, 1.0, 1.0, 0.9)
        ...
        '''
        # 标记功能的监听器
        self.render_window_interactor_axial.AddObserver("LeftButtonPressEvent", self.mark_point)
        self.render_window_interactor_coronal.AddObserver("LeftButtonPressEvent", self.mark_point)
        self.render_window_interactor_sagittal.AddObserver("LeftButtonPressEvent", self.mark_point)'''
    ...
    def on_mouse_move(self, obj, event):
        if not (self.marking) and not (self.erasing) and not(self.measuring) and not(self.measuring_angle) and not (self.picking):
            interactor = obj
        ...
        if self.measuring_horizontal_angle:
            ...
            if renderer is not None:
                ...
                if self.click == 0:
                    ...
                    if obj == self.render_window_interactor_axial:
                        self.point3 = self.rotate_coordinate(self.width,world_pos[1],world_pos[2],0,0,self.rotate_z_input.value())
                    elif obj == self.render_window_interactor_coronal:
                        self.point3 = self.rotate_coordinate(self.width,world_pos[1],world_pos[2],0,0,self.rotate_z_input.value())
                    elif obj == self.render_window_interactor_sagittal:
                        self.point3 = self.rotate_coordinate(world_pos[0],self.height,world_pos[2],0,0,self.rotate_z_input.value())
    ...
    def reset_measurement(self):
        ...
        self.render_window_interactor_axial.RemoveObservers(self.viewer_observer)
        self.render_window_interactor_coronal.RemoveObservers(self.viewer_observer)
        self.render_window_interactor_sagittal.RemoveObservers(self.viewer_observer)
    ...
    def set_physical_origin(self):
        ...
        self.update_physical_position_label_map(self.origin_physical[0],self.origin_physical[1],self.origin_physical[2])
...
# 以图像中心点为中心进行旋转
    ...
    def coordinate_planes_switch(self):
        ...
        if self.coords_plane_display:
            ...
            self.renderer_3d.AddActor(self.xy_plane_3d_actor)
            self.renderer_3d.AddActor(self.yz_plane_3d_actor)
            self.renderer_3d.AddActor(self.xz_plane_3d_actor)
